...
\item če $X \in T_2$, je retrakt zaprt.

\item (D) \(X\) ima \textbf{LNT} (lastnost negibne točke), če ima \(\forall\) zvezna preslikava \(X \rightarrow X\) negibno točko
    ...
    % \begin{wraptable}{R}{0.5\textwidth}
        ...
        \begin{table}[htbp]
            \begin{tabular}{l}
                ...
                $D_n: D \subseteq \mathbb{R}^n$  topološki $k$-disk za $0 \textless k \leq n$, torej $D \approx B^k$. $D$ ne deli $\mathbb{R}^n$ \\
            \end{tabular}
            \begin{tabular}{|l|}
                \hline
                $\chi(nT) = 2 - 2n$ \\
...
\item Posledica: $A,B \subseteq \mathbb{R}^n, A \approx B \Rightarrow Int(A) \approx Int(B)$ in $Meja(A) \approx Meja(B)$
...
\begin{itemize}
        ...
        \item $\bold{2}$.~disj unija (največ) števno mnogo $n$-mnt je $n$-mnt 
        \item $\bold{3}$.~Rob mnt: $M$ $n$-mnt,
        $int(M)^{odp} \subseteq M \Rightarrow \partial M^{zap} \subseteq M$
        \item $\bold{Izrek}$: (o odprti preslikavi za mnt) $M$, $N$ $n$-mnt, $V^{odp} \subseteq int(M), f: V \rightarrow N$ zv., inj.~$\Rightarrow f$ odp in $f_\ast (V) \subseteq int(N)$
        \item  $\bold{Izrek}$: $M$ $n$-mnt, $\partial M \neq \emptyset \Rightarrow \partial M$ $(n-1)$-mnt s praznim robom $(\partial (\partial M) = \emptyset)$; 
        če je $M$ kompaktna $\Rightarrow \partial M$ sklenjena
        (op: rob kompaktne ploskve je disjunktna unija krožnic) 
        \quad $\partial M$ ne deli $M$: $M$ povezana $\Rightarrow M \setminus \partial M$ povezana 
        $\bold{4}$.~Produkt mnt: 
        
        \item $\bold{Izrek}$: $M$ $m$-mnt, $N$ $n$-mnt $\Rightarrow M \times N$ je ($m+n$)-mnt, $int(M \times N) = int(M) \times int(N), \partial (M \times N) = \partial M \times N \cup M \times \partial N$
        
        \item $\bold{5}$.~Zlepek mnt: Def: $N$ $n$-mnt, $L$ $l$-mnt; $L \cap \partial N = \partial L \Rightarrow L$ prav vložena | $L$ lokalno ploščata ali podmnt, če je prav vložena in $\forall x \in L \exists$ evklidska okolica
        ...
        \item $\bold{Izrek}: N_1, N_2$ $n$-mnt, $L_i \subseteq \partial N_i$ ($n-1$)-mnt in $\partial L_i$ lokalno ploščat v $\partial N_i$ \quad $h: L_1 \rightarrow L_2$ homeo $\Rightarrow$ zlepek $N_1 \cup_h N_2 = N$
        ...
        \item $\bold{6}$.~Razrez mnt: $N$ $n$-mnt, $L \subseteq N$ lokalno ploščata ($n-1$)-podmnt $\Rightarrow$ lahko $N$ prerežemo vzdolž $L$ 
        
        \item $\bold{7}$. Povezana vsota: Def: $M,N$ n-mnt, $D \subseteq int(M), E \subseteq int(N)$
...
\item 
    \begin{itemize}
        \item $A, B \subseteq\mathbb{R}^n$ zap, $A, B \in AE(\mathcal{N})$: $|A \cap B| = 1 \Rightarrow A \cup B \in AE(\mathcal{N})$
        \item $A, B \subseteq\mathbb{R}^n$
        zap, $A, B \in AE(\mathcal{N})$:
        $A \cap B \in AE(\mathcal{N})\Rightarrow A \cup B \in AE(\mathcal{N})$ 
        % \newline
        \item $X$ kompaktna mnt: $\partial X \neq \emptyset, i: \partial X \rightarrow X$ inkluzija $\Rightarrow$ zlepek $X \cup_{\partial X} X := X \cup _i X$ kompaktna mnt brez roba (imenuje se podvojitev mnogoterosti X)
    \end{itemize}
% \item \begin{itemize}
%                 \item $\chi(nT) = 2 - 2n$, 
%                 \item $\chi(nP) = 2 - n$, 
%                 \item $\chi(S^2) = \chi(0T) = 2$ 
%                 \item $\chi(M) = 0$
%                 \item $K \approx 2P$, 
%                 \item $nT$ so orientabilni, $nP$ niso orientabilni
%             \end{itemize} 
\end{enumerate}
