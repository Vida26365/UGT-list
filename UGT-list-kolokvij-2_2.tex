...
\setlength\lineskip{0pt}
        \begin{enumerate}
            \item (p) Homeomorfizem $f: [a,b]\to[c,d]$ podamo kot $f(x)=\frac{d-c}{b-a}(x-a)+c$.
            \item (p) Homeomorfizem $f: (-1,1)\to\mathbb{R}$ podamo kot $f(x)=\frac{x}{1-|x|}$ in $g: \mathbb{R}\to(-1,1)$ kot $g(x)=\frac{x}{1+|x|}$.
...
\begin{enumerate}
    ...
    \item $\bold{1}$.~komponenta $n$-mnt je $n$-mnt (komponente so odprte, ker je mnt lok pov $\Leftarrow$ lok evklidskost) $\bold{2}$.~disj unija (največ) števno mnogo $n$-mnt je $n$-mnt $\bold{3}$.~Rob mnt: $M$ $n$-mnt,
    ...
    \newline $\bold{4}$.~Produkt mnt: $\bold{Izrek}$: $M$ $m$-mnt, $N$ $n$-mnt $\Rightarrow M \times N$ je ($m+n$)-mnt, $int(M \times N) = int(M) \times int(N), \partial (M \times N) = \partial M \times N \cup M \times \partial N$
    ...
    $\bold{6}$.~Razrez mnt: $N$ $n$-mnt, $L \subseteq N$ lokalno ploščata ($n-1$)-podmnt $\Rightarrow$ lahko $N$ prerežemo vzdolž $L$ $\bold{7}$. Povezana vsota: Def: $M,N$ n-mnt, $D \subseteq int(M), E \subseteq int(N)$
