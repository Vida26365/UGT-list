...
% \item ( ) \(T_2\) \(\implies\) lokalna kompaktnost\)
            
% \item Prostor $X$ je nepovezan
% \begin{itemize}[label={\(\iff\)}]
%     \setlength\itemsep{-1mm}
%     \item Prostor $X$ je disjunktna unija dveh nepraznih zaprtih podmnožic
%     \item V $X$ $\exists$ prava neprazna pomnožica, ki je hkrati odprta in zaprta
%     \item $\exists$ surjektivna preslikava $f:X\to\{0,1\}$
% \end{itemize}
...
\begin{tabular}{l}
    \hline
    Prostor $X$ je nepovezan \\
    \(\iff\) Prostor $X$ je disjunktna unija dveh nepraznih zaprtih podmnožic \\
    \(\iff\) V $X$ $\exists$ prava neprazna pomnožica, ki je hkrati odprta in zaprta \\
    \(\iff\) $\exists$ surjektivna preslikava $f:X\to\{0,1\}$ \\
% \end{tabular}
% \begin{tabular}{|l}
    % \setlength\itemsep{-1mm}
    \hline
    (I)  $X$ kompakten, $Y$ pa Hausdorffov prostor:\\
    \qquad Vsaka preslikava $f:X\to Y$ je zaprta. \\
    \qquad Vsaka injektivna preslikava $f:X\to Y$ je vložitev. \\
    \qquad Vsaka bijektivna preslikava $f:X\to Y$ je homeomorfizem. \\
\end{tabular}
\begin{tabular}{|l|}
    \hline
    metričnost $\implies$ (2-števnost $\iff$ seperabilnost) \\
    2-števnost \(\implies\) seperabilnost in 1-števnost \\
    % \item (I) Če je prostor $2$-števen, je tudi separabilen in $1$-števen (obrat ne velja).
    regularnost + 2-števnost \(\implies\) normalnost \\
    normalnost + 2-števnnost \(\implies\) metrizabilnost \\
    metrizabilnost \(\implies\) normalnost \\   
    2-števnost + regularnost \(\iff\) metrizabilnost \\
    kompaktnost + T2 \(\implies\) normalnost \\
    lokalna kompaktnost + \(T_2\) \(\implies\) regularnost \\
    \hline
\end{tabular}      
