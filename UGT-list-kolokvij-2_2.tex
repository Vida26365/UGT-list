...
\item  (I) $M$ $n$-mnt, $\partial M \neq \emptyset \Rightarrow \partial M$ $(n-1)$-mnt s praznim robom $(\partial (\partial M) = \emptyset)$; 
\item če je $M$ kompaktna $\Rightarrow \partial M$ sklenjena (op: rob kompaktne ploskve je disjunktna unija krožnic) 
\item $\partial M$ ne deli $M$ ($M$ povezana $\Rightarrow M \setminus \partial M$ povezana)
% \item $\bold{4}$.~Produkt mnt: 
\item (I) $M$ $m$-mnt, $N$ $n$-mnt $\Rightarrow M \times N$ je ($m+n$)-mnt
\item $\Int(M \times N) = \Int(M) \times \Int(N), \partial (M \times N) = \partial M \times N \cup M \times \partial N$
\item Zlepek mnt: (D) $N$ $n$-mnt, $L$ $l$-mnt; $L \cap \partial N = \partial L \Rightarrow L$ \textbf{prav vložena} 
\item $L$ lokalno ploščata ali podmnt, če je prav vložena in $\forall x \in L \exists$ evklidska okolica
